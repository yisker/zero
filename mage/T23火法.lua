...
function rotation:active_talents(args)
    ...
    if buff.runeOfPower.up() and (buff.firestarter.remains()>cooldown.meteor.duration() or not (getTalent(1,1) and getHP("target") >= 90)) or cooldown.runeOfPower.remains()>target.time_to_die() and action.runeOfPower.charges()<1 or (cooldown.meteor.duration()<cooldown.combustion.remains() or cooldown.combustion.ready()) and not talent.runeOfPower and (cooldown.meteor.duration()<firestarter.remains() or not talent.firestarter or not (getTalent(1,1) and getHP("target") >= 90)) then
...
function rotation:default_action()
    ...
    variable.fireBlast_pooling=talent.runeOfPower and cooldown.runeOfPower.remains()<cooldown.fireBlast.full_recharge_time() and (cooldown.combustion.remains()>variable.combustion_rop_cutoff or (getTalent(1,1) and getHP(tg) >= 90)) and (cooldown.runeOfPower.remains()<target.time_to_die() or action.runeOfPower.charges()>0) or cooldown.combustion.remains()<action.fireBlast.full_recharge_time()+cooldown.fireBlast.duration()*azerite.blasterMaster.enabled and not (getTalent(1,1) and getHP(tg) >= 90) and cooldown.combustion.remains()<target.time_to_die() or talent.firestarter and (getTalent(1,1) and getHP(tg) >= 90) and buff.firestarter.remains()<cooldown.fireBlast.full_recharge_time()+cooldown.fireBlast.duration()*azerite.blasterMaster.enabled
...
function bm_combustion_phase(args)
    ...
    -- actions.bm_combustion_phase+=/livingBomb,
    ...
    -- actions.bm_combustion_phase+=/runeOfPower,
    if buff.combustion.down() then
        if cast.able.runeOfPower() and cast.runeOfPower() then
            if ydebug.is_enabled then
                print(303)
                return 0
            else
    ...
    -- actions.bm_combustion_phase+=/fireBlast,use_while_casting=1,
    if buff.blasterMaster.down() and (talent.runeOfPower and action.runeOfPower.executing() and action.runeOfPower.execute_remains()<0.6 or (cooldown.combustion.ready() or buff.combustion.up()) and not talent.runeOfPower and not action.pyroblast.in_flight() and not action.fireball.in_flight()) then
        if cast.able.fireBlast and cast.fireBlast() then
            if ydebug.is_enabled then
                print(304)
                return 0
            else
    ...
    -- actions.bm_combustion_phase+=/call_action_list,name=
    self:active_talents()
    -- actions.bm_combustion_phase+=/combustion,use_off_gcd=1,use_while_casting=1,if azerite.blasterMaster.enabled and ((action.meteor.in_flight and action.meteor.in_flight_remains<0.2) or not talent.meteor.enabled or prev_gcd.1.meteor) and (buff.runeOfPower.up or not talent.runeOfPower.enabled)
    ...
    -- actions.bm_combustion_phase+=/fireBlast,use_off_gcd=1,if buff.blasterMaster.stack=1 and buff.hot_streak.down and not buff.pyroclasm.react and prev_gcd.1.pyroblast and (buff.blasterMaster.remains<0.15 or gcd.remains<0.15)
    -- actions.bm_combustion_phase+=/fireBlast,use_while_casting=1,if buff.blasterMaster.stack=1 and (action.scorch.executing and action.scorch.execute_remains<0.15 or buff.blasterMaster.remains<0.15)
    ...
    -- actions.bm_combustion_phase+=/fireBlast,use_while_casting=1,use_off_gcd=1,if buff.blasterMaster.stack>1 and (prev_gcd.1.scorch and not buff.hot_streak.up and not action.scorch.executing or buff.blasterMaster.remains<0.15)
...
-- actions.combustion_phase+=/call_action_list,name=bm_combustion_phase,if azerite.blasterMaster.enabled and talent.flame_on.enabled
-- actions.combustion_phase+=/runeOfPower,if buff.combustion.down
-- actions.combustion_phase+=/call_action_list,name=active_talents
-- actions.combustion_phase+=/combustion,use_off_gcd=1,use_while_casting=1,if (not azerite.blasterMaster.enabled or not talent.flame_on.enabled) and ((action.meteor.in_flight and action.meteor.in_flight_remains<=0.5) or not talent.meteor.enabled) and (buff.runeOfPower.up or not talent.runeOfPower.enabled)
...
-- actions.combustion_phase+=/fireBlast,use_off_gcd=1,use_while_casting=1,if (not azerite.blasterMaster.enabled or not talent.flame_on.enabled) and ((buff.combustion.up and (buff.heating_up.react and not action.pyroblast.in_flight and not action.scorch.executing) or (action.scorch.execute_remains and buff.heating_up.down and buff.hot_streak.down and not action.pyroblast.in_flight)))
