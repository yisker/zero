...
function cPlayer:new(unit)
    ...
    local function getSpells(spellTable)
        ...
        for spellType, spellTypeTable in pairs(spellTable) do
            -- Create spell type subtable in player.spell if not already there.
            ...
            for spellRef, spellID in pairs(spellTypeTable) do
                -- Assign spell to player.spell for the spell type
                self.spell[spellType][spellRef] = spellID
                -- Assign active spells to Abilities Subtable and base player.spell
    ...
    for k,v in pairs(self.spell.abilities) do
        ...
        if self.cooldown[k]     == nil then self.cooldown[k]        = {} end        -- Spell Cooldown Functions
    ...
    for k, v in pairs(self.power.list) do
        ...
        power.amount = function()
            if isDKRunes then
                ...
                return UnitPower("player",v)
            end
        end
        power.deficit = function()
            return UnitPowerMax("player",v) - UnitPower("player",v)
        end
        power.frac = function()
            if isDKRunes then
                local runeCount = 0
        ...
        power.max = function()
            return UnitPowerMax("player",v)
        end
        power.max_stack = function()
            return UnitPowerMax("player",v)
        end
        power.percent = function()
            if gUnitPowerMax("player",v) == 0 then
                return 0
            else
                return ((UnitPower("player",v) / UnitPowerMax("player",v)) * 100)
            end
        end
...
function cTarget:new(unit)
    local self = cPlayer:new()    
    self.time_to_die = getTimeToDie(unit)    
    
end

cPrev_gcd = {}
function cPrev_gcd(args)
    local self = {}
    self.1 = function(spellid)
        return getLastSpell() == spellid
    end   
